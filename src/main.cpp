...
Eigen::MatrixXd performLucasKanadeAffine_Eigen(const cv::Mat &It_opencv, const cv::Mat &It1_opencv, const Eigen::RowVectorXi &rect)
{
    Eigen::MatrixXd M = Eigen::MatrixXd::Identity(2, 3);
    Eigen::MatrixXd M_inv(2, 3);
    ...
    Eigen::VectorXd bbox_Xs = Eigen::VectorXd::LinSpaced(bbox_W, x1, x2);
    Eigen::VectorXd bbox_Ys = Eigen::VectorXd::LinSpaced(bbox_H, y1, y2);

    Eigen::VectorXd flat_bbox_Xs = bbox_Xs.replicate(bbox_Ys.size(), 1);
    Eigen::VectorXd flat_bbox_Ys = bbox_Ys.replicate(1, bbox_Xs.size()).transpose().reshaped();

    // Extract template from image It
    Eigen::MatrixXd It;
    cv::cv2eigen(It_opencv, It);
    Eigen::VectorXd flat_template = It.block(y1, x1, bbox_H, bbox_W).reshaped<Eigen::RowMajor>();
    ...
    Eigen::MatrixXd warped_target, gradI_xp, gradI_yp;
    Eigen::VectorXd flat_gradI_xp, flat_gradI_yp;

    Eigen::ArrayXd column_arr(bbox_W * bbox_H);
    ...
    for (int itr = 0; itr < maxIterations; itr++) // TODO: Make it maxIterations again
    {
        ...
        Eigen::VectorXd flat_warped_target = warped_target.block(y1, x1, bbox_H, bbox_W).reshaped<Eigen::RowMajor>();

        // 2. Compute Error [y1 : (y2 + 1), x1 : (x2 + 1)]
        Eigen::VectorXd b = flat_template - flat_warped_target;
        ...
        */
        Eigen::MatrixXd A(bbox_W * bbox_H, 6);

        // column_arr = flat_gradI_xp.array() * flat_bbox_Xs.array();
        column_arr = flat_gradI_xp.array() * flat_bbox_Xs.array();
        A.col(0) = Eigen::Map<Eigen::MatrixXd>(column_arr.data(), A.rows(), 1);

        column_arr = flat_gradI_yp.array() * flat_bbox_Xs.array();
        A.col(1) = Eigen::Map<Eigen::MatrixXd>(column_arr.data(), A.rows(), 1);

        column_arr = flat_gradI_xp.array() * flat_bbox_Ys.array();
        A.col(2) = Eigen::Map<Eigen::MatrixXd>(column_arr.data(), A.rows(), 1);

        column_arr = flat_gradI_yp.array() * flat_bbox_Ys.array();
        A.col(3) = Eigen::Map<Eigen::MatrixXd>(column_arr.data(), A.rows(), 1);

        A.col(4) = Eigen::Map<Eigen::MatrixXd>(flat_gradI_xp.data(), A.rows(), 1);

        A.col(5) = Eigen::Map<Eigen::MatrixXd>(flat_gradI_yp.data(), A.rows(), 1);
...
void useMomentumOnOcclusion(bool is_occluded, Eigen::MatrixXd &M, Eigen::MatrixXd velocities, int img_idx)
{
    if (is_occluded)
    {
        assert(img_idx >= 2); // Doesn't handle if object occluded in the first two frames
        Eigen::MatrixXd frame_velocity = velocities.block(img_idx, 0, 2, 2).colwise().mean();
        frame_velocity(0, 1) = -frame_velocity(0, 1);
        M.block(0, 0, 2, 2) = Eigen::Matrix2d::Identity();
...
{
    std::vector<bool> occlusions = readOcclusions();

    Eigen::MatrixXd velocities(NUM_IMAGES - 1, 2);

    Eigen::RowVectorXd initial(4);
    ...
    */
    ...
    Eigen::MatrixXd rects(NUM_IMAGES, 4);
    rects.row(0) = initial;

    Eigen::MatrixXd M(2, 3);       // Affine matrix
    Eigen::MatrixXd corners(3, 2); // Homogenous coordinates
    Eigen::RowVectorXd rect, new_rect;
    Eigen::RowVectorXd frame_velocity;
    ...
    for (int img_idx = 0; img_idx < NUM_IMAGES - 1; img_idx++)
    {
         ...
         */
        M = performLucasKanadeAffine_Eigen(It, It1, rect.cast<int>());
...
int main()
{
    ...
    // std::vector<std::vector<int>> groundtruths = readGroundtruths();
    // visualizeGroundtruths(groundtruths);
